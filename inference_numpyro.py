...
def define_model(exp_data, not_observed):
    ...
    theta_q = numpyro.sample('theta_q', dist.HalfNormal(5).expand([2, num_agents]))
    theta_r = numpyro.sample('theta_r', dist.HalfNormal(5).expand([2, num_agents]))
    ...
    observed = jnp.delete(exp_data['choices'], jnp.asarray(not_observed))
    
    probabils = jnp.delete(probs, jnp.asarray(not_observed), axis = 0)

    numpyro.sample('like', dist.Categorical(probs=probabils), obs=observed)
